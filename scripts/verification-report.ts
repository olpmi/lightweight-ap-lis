...
/**
 * Identifies the CI run that produced this report, when there is one.
 *
 * A table of test results is only evidence if a reader can tell where it came
 * from. Without this the file records a commit but not the run, so a report
 * generated on a workstation and one generated by
 * `.github/workflows/verification.yml` are indistinguishable — and only the second
 * is reproducible by someone else.
 */
function describeRun(): { url: string | null; attempt: string | null } {
  const server = process.env.GITHUB_SERVER_URL;
  const repository = process.env.GITHUB_REPOSITORY;
  const runId = process.env.GITHUB_RUN_ID;
  if (!server || !repository || !runId) return { url: null, attempt: null };
...
function describeEnvironment(): {
  ...
  commit: string;
  node: string;
  run: ReturnType<typeof describeRun>;
} {
...
function renderMarkdown(
  ...
  const lines = [
    ...
    // States where the run happened, so a reproducible CI artifact is not mistaken
    // for a developer-machine one, and vice versa.
    environment.run.url
      ? `- CI run: ${environment.run.url}` +
        (environment.run.attempt ? ` (attempt ${environment.run.attempt})` : '')
      : '- CI run: none — generated outside CI, so these numbers are not independently reproducible',
